...
plantTypes: { name: string; growthConditions: (x: number, y: number, gridState: GridState) => boolean }[];
...
export default class GameScene extends Phaser.Scene {
    constructor() {
        ...
        this.undoStack = [] as Array<any>; // Undo stack
        this.redoStack = [] as Array<any>; // Redo stack
    ...
    clearPlantSprites(): void {
        this.grid.forEach((cell: { plantSprite: Phaser.GameObjects.Sprite | null }) => {
            if (cell.plantSprite) {
            cell.plantSprite.destroy();
    ...
    restorePlantSprites(plants: Array<{ x: number; y: number; plantType: number; growthLevel: number; waterLevel: number; sunLevel: number }>): void {
        plants.forEach(({ x, y, plantType, growthLevel, waterLevel, sunLevel }) => {
          const cell = this.grid.find((c: { x: number; y: number }) => c.x === x && c.y === y);
          if (cell) {
            const frameIndex: number = plantType !== 3 ? growthLevel - 1 : 6 + (growthLevel - 1);
    ...
    restoreState(state: {
        ...
        plants: Array<{ x: number; y: number; plantType: number; growthLevel: number; waterLevel: number; sunLevel: number }>;
      }): void {
    ...
    autoSaveGame(): void {
        try {
          ...
          const compressedState: string = LZString.compress(JSON.stringify(gameState)); // Use global LZString
      ...
      saveGame(slot: number): void {
        try {
          const gameState: Record<string, any> = this.getCurrentState();
          const compressedState: string = LZString.compress(JSON.stringify(gameState));
      ...
      sowPlant(): void {
        ...
        if (cell && this.gridState.getPlantType(cell.x, cell.y) === 0) {
          const plantTypeIndex: number = Phaser.Math.Between(0, plantTypes.length - 1);
      ...
      plantGrowth(x: number, y: number, plantTypeIndex: number): void {
        const plantType = plantTypes[plantTypeIndex];
        const newGrowthLevel: number = plantType.grow(x, y, this.gridState, this.gameTime);
      ...
      addPlantType(name: string, growthConditions: (x: number, y: number, gridState: GridState, gameTime: number) => boolean): number {
